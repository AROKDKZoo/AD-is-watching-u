...
var Engine = function (bg) {
    ...
    //项大小
    this.ItemSize = new Point(72, 72);
    //整个画布大小
    this.BackGroundSize = new Point(900,600);

    //配置信息
    this.Config = new LLHConfig();

    //操作区域大小
    this.ItemAreaPer = 0.8;
...
Engine.prototype.SetLevel = function (level) {
    ...
    {
        alert('テンアゲﾞ～ブチアゲ～カマアゲ～アリるんパワー☆アリアリのラブ～ア·ゲ·ル☆');
        return;
    }
...
Engine.prototype.RefreshItem = function (item) {
    ...
    if (item.Visibled) {
        ...
        if (item.Selected) { //如果被选择
            this.Context.strokeStyle = '#FF4500'; // orange
...
Engine.prototype.CreateCanvas = function () {
    ...
    this.BackItemContainer.innerHTML = "请更换支持HTML5的浏览器再进行游戏喔^ ^";
...
Engine.prototype.InitEngine = function () {
    ...
    if (!this.ScroeArea) {
        ...
        this.ScroeArea.setAttribute('style', 'border: 1px solid red;position: absolute;left:0px;top:0px;height:22px;width:80px;color:SandyBrown; font-weight:600;background-color:MintCream;');
    ...
    if (!this.LevelArea) {
        ...
        this.LevelArea.setAttribute('style', 'border: 1px solid red;position: absolute;left:' + (this.BackGroundSize.X - 80) + 'px;top:0px;height:22px;width:80px;color:SandyBrown; font-weight:600;background-color:MintCream;');
